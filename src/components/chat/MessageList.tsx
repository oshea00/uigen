...
function getToolLabel(toolName: string, args: Record<string, unknown> | undefined, completed: boolean) {
  const filename = (args?.path as string)?.split("/").pop();

  if (toolName === "str_replace_editor") {
    const verb = args?.command === "create"
      ? completed ? "Created" : "Creating"
      : completed ? "Edited" : "Editing";
    return filename ? `${verb} ${filename}` : `${verb} code`;
  }

  if (toolName === "file_manager") {
    const verb = args?.command === "delete"
      ? completed ? "Deleted" : "Deleting"
      : args?.command === "rename"
        ? completed ? "Renamed" : "Renaming"
        : completed ? "Managed" : "Managing";
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const toolLabel = getToolLabel(tool.toolName, tool.args, !!isComplete);
                            return (
